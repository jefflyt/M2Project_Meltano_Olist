...
# dbt-run.sh - Olist dbt Transform Convenience Script
...
# Project info
PROJECT_NAME="dbt_transform_olist"
PROFILE_NAME="dbt_transform_olist"
PROFILES_DIR="$(pwd)"
DEFAULT_TARGET="dev"

echo -e "${CYAN}=================================================================${NC}"
echo -e "${CYAN}🚀 Olist dbt Transform - Using Local Profiles${NC}"
echo -e "${CYAN}=================================================================${NC}"
...
echo -e "${BLUE}[${PROJECT_NAME}] Running: dbt ${COMMAND} --profiles-dir . $@${NC}"
echo ""

# Run the dbt command with local profiles
if dbt ${COMMAND} --profiles-dir . "$@"; then
    echo ""
